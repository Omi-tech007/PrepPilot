...
/**
 * JEEPLANET PRO - v17.0 (Fullscreen + Distraction Free Mode)
 */
...
// --- 2. FOCUS TIMER (Distraction Free + Fullscreen) ---
const FocusTimer = ({ data, setData, onSaveSession }) => {
  ...
  const toggleFullscreen = () => {
    if (!document.fullscreenElement) {
        containerRef.current.requestFullscreen().then(() => setIsFullscreen(true)).catch(err => alert("Fullscreen blocked"));
    } else {
    ...
    >
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-20">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={toggleFullscreen} className="p-2 bg-[#18181b]/90 border border-white/10 rounded-full text-gray-400 hover:text-white transition" title="Fullscreen">
      ...
      <AnimatePresence>
          {showSettings && (
              <motion.div initial={{opacity:0, y:-20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="absolute top-20 right-4 z-30 bg-[#18181b] border border-white/10 p-4 rounded-xl shadow-2xl w-72">
                  ...
                  <input type="text" placeholder="Paste Image URL here..." className="w-full bg-white/5 border border-white/10 rounded p-2 text-xs text-white mb-2 outline-none focus:border-violet-500" value={data.bgImage || ''} onChange={(e) => setData({...data, bgImage: e.target.value})} />
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-8 z-10">
         ...
         {/* --- 1. TOGGLES DISAPPEAR WHEN ACTIVE --- */}
         {!isActive && (
         ...
         <div className="bg-[#18181b]/90 backdrop-blur border border-white/10 p-2 rounded-2xl flex items-center gap-4 shadow-2xl transition-all duration-500">
            ...
            {/* --- 2. SUBJECT SELECTOR DISAPPEARS (REPLACED BY TEXT) --- */}
            {isActive ? (
                <div className="px-6 py-3 font-bold text-violet-400 flex items-center gap-2 bg-white/5 rounded-xl border border-white/5">
                    <div className="w-2 h-2 rounded-full bg-violet-500 animate-pulse"></div>
                    Studying: {selectedSub}
                </div>
            ) : (
